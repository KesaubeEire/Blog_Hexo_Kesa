...
- 策略
  ...
  - 经验风险最小化|结构风险最小化函数
    - 经验:损失量取平均 适用于大数据的情况
    - 结构:有个 λ - 0.1 1 10 100 ….一个惩罚项的系数,防止过拟合
- 具体算法
...
- 分类性能指标:
  ...
  - 正确率（Accuracy）：预测正确的结果所占总数的比例。
    ...
    - 正确率高并不能说明分类器效果好。比如识别是否一个人是否有艾滋病，就算全部预测为负样本，那正确率也有 99%
      以上（因为每万人才有 6 个艾滋病人），但是这种高正确率的分类器其实并没有任何作用。
...
- 思想：条件概率分布；
  对一个观测序列 X 找到使得条件概率 P(X|Y)最大的标记序列 Y；
...
self.b = 0
self.l_rate = 0.1 # self.data = data
